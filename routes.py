...
def register_post():
    ...
    new_user = User(username=username, passhash=password_hash, name=name) # type: ignore
...
def profile_post():
    ...
    user = User.query.get(session['user_id'])
    if not check_password_hash(user.passhash, cpassword): # type: ignore
        flash('Incorrect password')
        return redirect(url_for('profile'))
    
    if username != user.username: # type: ignore
    ...
    user.passhash = new_password_hash # type: ignore
...
def add_product_post():
    ...
    product = Product(name=name, price=price, category=category, quantity=quantity, man_date=man_date) # type: ignore
...
def edit_product_post(id):
    ...
    product.man_date = man_date  # type: ignore
    db.session.commit()
...
def add_to_cart(product_id):
    ...
    if cart:
        ...
        cart = Cart(user_id=session['user_id'], product_id=product_id, quantity=quantity) # type: ignore
        db.session.add(cart)
...
def checkout():
    ...
    transaction = Transaction(user_id=session['user_id'], datetime=datetime.now()) # type: ignore
    for cart in carts:
        order = Order(transaction=transaction, product=cart.product, quantity=cart.quantity, price=cart.product.price) # type: ignore
